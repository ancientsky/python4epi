...
class Ch02EpiCurveScene(EpiBaseScene):
    ...
    def show_title(self, duration: float = 3.0, **kwargs) -> None:
        ...
        self.show_title_card("matplotlib 流行曲線", "從入門到疫調等級", duration=duration)
    ...
    def show_main_summary(self, duration: float = 5.0, **kwargs) -> None:
        ...
        points = VGroup(
            Text("1. fig, ax = plt.subplots() 建立畫布", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("2. value_counts + sort_index 算每日病例", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("3. mdates 格式化日期軸", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("4. reindex 補零避免斷裂", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("5. CDC 風格：方塊長條 + 格線", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("6. 標題、軸標籤、tight_layout", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
    ...
    def show_extra_banner(self, duration: float = 2.0, **kwargs) -> None:
        ...
        banner = ExtraExampleBanner("額外範例：麻疹群聚流行曲線")
    ...
    def show_blindspot_banner(self, duration: float = 2.0, **kwargs) -> None:
        ...
        banner = BlindSpotBanner("流行曲線經典坑 3 選")
        self.show_section_banner(banner, duration=duration)
