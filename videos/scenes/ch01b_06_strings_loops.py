...
class Ch01bStringsLoopsScene(EpiBaseScene):
    ...
    def show_title(self, duration: float = 3.0, **kwargs) -> None:
        self.show_title_card("字串與迴圈進階", "清理疫調資料的瑞士刀", duration=duration)
    ...
    def show_main_summary(self, duration: float = 4.0, **kwargs) -> None:
        ...
        points = VGroup(
            Text("1. strip() 去空白、split() 切分", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("2. replace() 取代字串內容", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("3. for + range() 跑固定次數", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("4. enumerate() 同時拿索引和值", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
            Text("5. in 檢查成員是否存在", font=FONT_CJK, font_size=22, color=TEXT_PRIMARY),
    ...
    def show_extra_banner(self, duration: float = 2.0, **kwargs) -> None:
        banner = ExtraExampleBanner("額外範例：清理 TB 通報檢驗結果")
    ...
    def show_outro(self, duration: float = 3.0, **kwargs) -> None:
        ...
        sub = Text("管理 Python 版本和套件！", font=FONT_CJK, font_size=22, color=TEXT_SECONDARY).next_to(heading, DOWN, buff=0.4)
