...
## 🔋 超白話特別篇：用「手機電池大賽」看懂存活分析

> 覺得存活分析好難？別怕。這一段我們把所有嚇人的名詞，換成你每天都在用的**手機電池**。看完再回頭看下面那幾張圖，你會發現：喔～原來就這樣！

### 一場手機電池的比賽

想像全班每人拿一支充飽電（100%）的手機來比賽：**看誰的手機能撐最久才沒電關機。**

- 存活分析在乎的不只是「會不會關機」（每支遲早都會），而是**「撐了多久才關機」**——這正是它厲害的地方：**不只看有沒有發生，還看「多久之後」發生。**
- 手機關機（電量歸零）＝ 我們在等的那個「**事件（event）**」。在醫院裡，事件可能是「死亡」「痊癒出院」或「復發」。
- 從 100% 撐到關機的小時數 ＝ **存活時間**。

### 最重要的一招：有些手機「還沒關機你就下課了」

比賽到一半，下課鐘響、你得回家——這時還有幾支手機**還開著**。你會怎麼記？

- 寫「0 小時」？不對，它明明撐很久。
- 寫「壞掉／沒資料」？也不對，它好得很。
- 正確是：**「這支至少撐了 12 小時，但實際能撐多久，我沒看到。」**

這種「只知道下限、沒看到結局」的資料，就叫 **設限（censored）**。

> 🔑 一句話記住：**設限 ≠ 關機，也 ≠ 壞掉。它是「我們只看到一半的故事」。** 存活分析最聰明的地方，就是連這種手機也能算進去，不會白白丟掉。

```{figure} images/km_battery_metaphor.svg
:name: fig-km-battery
:alt: 用手機電池比喻 Kaplan-Meier 曲線：8 支手機中 6 支在不同時間關機、2 支下課時還開著（設限），對應到一條往下踏階的存活曲線與中位存活時間
:width: 100%

上排 8 支手機、下排一條往下走的樓梯——**每有一支手機關機，樓梯就往下踏一階**。
```

### KM 曲線：一條會「下樓梯」的線

橫軸放時間，縱軸放「**現在還開著的手機比例**」。一開始 100%（大家都開著），每關機一支，線就往下**踏一階**——所以 Kaplan-Meier 曲線長得像一座**往下走的樓梯**：

- **樓梯越陡** ＝ 手機一支接一支很快關機（**存活差**）。
- **樓梯越平** ＝ 很耐用、撐很久（**存活好**）。
- 線**穿過 50% 的那個時間**，叫 **中位存活時間**——「一半的手機在這時已經關機」。
- 圖上小小的直線或「＋」記號 ＝ **設限**的手機（那時被你收走了），所以線在那裡**不會**往下踏。

### 兩個廠牌比一比：Log-rank 檢定

把手機分成「蘋果牌」和「香蕉牌」，各畫一條樓梯：

- 若香蕉牌的樓梯**一直踩在蘋果牌下面**（掉得比較快），代表香蕉牌比較不耐用。
- 但差距**會不會只是運氣**？**Log-rank 檢定**就是在問：**「這兩條樓梯的差距，是真的，還是碰巧？」** 它給你一個 **p 值**，**p < 0.05** ≈「這差距不太可能只是運氣」。

### 「耗電速度」：Hazard 與風險比 HR

**Hazard** 聽起來嚇人，其實就是**「這一瞬間的耗電速度」**——手機現在掉電多快。把兩牌的耗電速度相除，就是**風險比（HR）**：

- **HR = 2**：香蕉牌的電**掉得快兩倍**（風險兩倍）。
- **HR = 1**：兩牌一樣快（沒差）。
- **HR = 0.5**：香蕉牌**只掉一半速度**（比較耐用，是**保護因子**）。

看 **HR 森林圖**同一招：點在 **1 的右邊** ＝ 風險較高；**1 的左邊** ＝ 較低（保護）。點旁的**橫線是信賴區間**，這條線**沒碰到 1**，代表「差異是真的」。
...
# 兩個廠牌各 40 支手機，模擬「撐到關機的小時數」
apple = rng.gamma(shape=9, scale=1.1, size=40)    # 蘋果牌：比較耐用
banana = rng.gamma(shape=5, scale=1.1, size=40)   # 香蕉牌：掉電快

# 觀察只到第 12 小時就「下課」→ 超過的算設限（還開著）
CUTOFF = 12
def make(times, brand):
    return pd.DataFrame({
        "brand": brand,
        "hours": np.minimum(times, CUTOFF),
        "died": (times <= CUTOFF).astype(int),   # 1=關機(事件), 0=設限
    })
phones = pd.concat([make(apple, "蘋果牌"), make(banana, "香蕉牌")], ignore_index=True)

fig, ax = plt.subplots(figsize=(7, 4.5))
kmf = KaplanMeierFitter()
for brand, color in [("蘋果牌", "#6A9BCC"), ("香蕉牌", "#D97757")]:
    g = phones[phones["brand"] == brand]
    kmf.fit(g["hours"], g["died"], label=brand)
    kmf.plot_survival_function(ax=ax, color=color)
    print(f"{brand} 中位存活（一半關機的時間）：{kmf.median_survival_time_:.1f} 小時")

ax.axhline(0.5, ls="--", color="#6B6B6B", lw=1)   # 50% 參考線 → 中位存活
ax.set_xlabel("時間（小時）")
ax.set_ylabel("還開著的手機比例")
ax.set_title("手機電池大賽：Kaplan-Meier 存活曲線")
plt.tight_layout()
plt.show()

# 兩牌差距是真的，還是運氣？→ Log-rank 檢定
a = phones[phones.brand == "蘋果牌"]
b = phones[phones.brand == "香蕉牌"]
res = logrank_test(a["hours"], b["hours"], a["died"], b["died"])
verdict = "差距是真的（p < 0.05）" if res.p_value < 0.05 else "看不出顯著差異"
print(f"Log-rank p 值 = {res.p_value:.4f} → {verdict}")
```

跑跑看：蘋果牌的中位存活大約是香蕉牌的兩倍，而且 log-rank 的 p 值很小——**兩牌真的有差**。恭喜，你剛剛已經完整做了一次存活分析！
...
| 線上的小記號（＋） | 設限：只看到一半的故事 |
| p < 0.05 | 差距不太可能只是運氣 |
| HR > 1（點在 1 右邊） | 風險較高（掉電更快） |
| HR < 1（點在 1 左邊） | 保護作用（更耐用） |
| 信賴區間沒碰到 1 | 這個差異是真的 |

### 回到真實：手機 → 病人

把「手機」換成「病人」、「關機」換成「死亡」、「廠牌」換成「疾病嚴重度」——你剛剛學的每一招，就是本章 Step 2–6 在做的事。現在回頭再看一次那幾張 KM 曲線、log-rank 與 HR 森林圖，是不是順眼多了？😉
